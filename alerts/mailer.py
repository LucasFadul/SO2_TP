...
def build_alert_message(alarm: Mapping[str, object]) -> EmailMessage:
...
def send_alert(alarm: Mapping[str, object], dry_run: bool = True) -> dict:
    message = build_alert_message(alarm)
    if dry_run:
        return {"accion": "send_alert", "dry_run": True, "ok": True, "subject": message["Subject"]}

    host = os.getenv("HIPS_SMTP_HOST", "localhost")
    port = int(os.getenv("HIPS_SMTP_PORT", "25"))
    with smtplib.SMTP(host, port) as smtp:
        smtp.send_message(message)
    return {"accion": "send_alert", "dry_run": False, "ok": True, "subject": message["Subject"]}
